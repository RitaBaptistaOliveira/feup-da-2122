...
template<class T>
std::vector<T> Graph<T>::dfs(const T &s) {
    ...
    return visited
    vertexSet[findVertexIdx(s)].visited = true;
    for (auto e : vertexSet[findVertexIdx(s)].adj) {
        int w = e.dest;
        if (!vertexSet[findVertexIdx(w)].visited)
            dfs(w);
    }
}

template<class T>
void Graph<T>::dfs_visit(const T &s) {
